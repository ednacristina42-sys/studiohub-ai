...
import { Plus, Trash2, Pencil, Search, ShoppingBag, Tags, ImageOff, X } from "lucide-react";
...
const emptyForm = { name: "", description: "", category: "", price: "", image_url: "", active: true };
...
export default function Store() {
  ...
  const openEdit = (p) => {
    setEditing(p);
    setForm({ name: p.name, description: p.description || "", category: p.category || "", price: p.price, image_url: p.image_url || "", active: p.active });
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    const payload = { name: form.name, description: form.description, category: form.category, price: Number(form.price) || 0, image_url: form.image_url, active: form.active };
  ...
  const addCategory = async () => {
    ...
    await api.post("/store/categories", { name: newCat.trim() });
    setNewCat(""); toast.success("Categoria adicionada");
    api.get("/store/categories").then((r) => setCategories(r.data));
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Gere os produtos que vendes aos teus clientes.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" data-testid="manage-categories-btn" onClick={() => setCatOpen(true)} className="rounded-lg gap-2"><Tags className="h-4 w-4" /> Categorias</Button>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input data-testid="store-search" placeholder="Pesquisar produtos..." value={search} onChange={(e) => setSearch(e.target.value)} className="h-10 pl-9" />
        </div>
        <Select value={catFilter} onValueChange={setCatFilter}>
          <SelectTrigger data-testid="store-category-filter" className="h-10 w-48"><SelectValue /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger data-testid="store-status-filter" className="h-10 w-36"><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      {filtered.length === 0 ? (
        <Card className="p-16 border-dashed flex flex-col items-center text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Adiciona o teu primeiro produto para começar a vender.</p>
        </Card>
      ) : (
                ...
                <div className="p-4 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      {p.category && <Badge variant="secondary" className="rounded-full text-[10px] font-normal mt-1">{p.category}</Badge>}
                    </div>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm">{form.active ? "Ativo (visível na loja)" : "Inativo (oculto)"}</span>
            </div>
          </div>
      ...
      <Dialog open={catOpen} onOpenChange={setCatOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className="font-display font-medium">Categorias de produtos</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="flex gap-2">
              <Input data-testid="new-category-input" value={newCat} onChange={(e) => setNewCat(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addCategory()} placeholder="Nova categoria..." className="h-10" />
              <Button data-testid="add-category-btn" onClick={addCategory} className="rounded-lg">Adicionar</Button>
            </div>
            <div className="flex flex-wrap gap-2">
              {categories.length === 0 ? <p className="text-sm text-muted-foreground">Sem categorias.</p> : categories.map((c) => (
                <Badge key={c.id} variant="secondary" data-testid={`category-chip-${c.id}`} className="rounded-full gap-1 pr-1 py-1">
                  {c.name}
                  <button onClick={() => removeCategory(c.id)} className="ml-1 rounded-full hover:bg-destructive/20 p-0.5" data-testid={`delete-category-${c.id}`}><X className="h-3 w-3" /></button>
                </Badge>
              ))}
            </div>
          </div>
